...
from core.odds_parser import scrape_over_under, scrape_over_under_half, scrape_nowscore_both
...
SCRAPER_MAP = {
    "full": scrape_over_under,
    "half": scrape_over_under_half,
}


def _backfill_sibling(schedule_id: int, comp, season: str, is_cup: bool,
                      cid: int, st: str, item, match_time: str) -> bool:
    """Write the sibling odds type (asian) from a nowscore request,
    ...
    match_dir = os.path.join(ODDS_DIR, "asian", subdir, dir_name, season, str(schedule_id))
    filename = f"{cid}.json" if st == "full" else f"{cid}_half.json"
    path = os.path.join(match_dir, filename)
    if os.path.isfile(path):
        return False
    rec = odds_store.build_record(schedule_id, cid, item.company_name, "asian",
...
def process_match_odds(schedule_id: int, comp, season: str, is_cup: bool,
                       ...
                       match_time: str = ""):
    """Fetch Over/Under odds for one match."""
    subdir = "cups" if is_cup else "leagues"
    dir_name = utils.schedule_dir_name(comp)
    season_dir = os.path.join(ODDS_DIR, "over_under", subdir, dir_name, season)
    ...
    for cid in companies:
        for st in subtypes:
            ...
            print(f"      [over_under/{st}] SID={schedule_id} cid={cid}")
            sys.stdout.flush()
            ...
            try:
                ...
                if item is None or not item.changes:
                    ...
                    item = both["over_under"]
                    if both["asian"] and both["asian"].changes:
                        if _backfill_sibling(schedule_id, comp, season, is_cup,
                                             cid, st, both["asian"], match_time):
                            print(f"        backfilled asian/{st} cid={cid}")
...
def run_competition(comp, is_cup=False, seasons=None,
    ...
    """Scrape Over/Under for a competition."""
    ...
    print(f"\n{'='*60}")
    print(f"OVER/UNDER: {name} (ID={cid})")
    print(f"{'='*60}")
